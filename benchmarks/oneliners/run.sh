...
oneliners_agg() {
    ...
    for script_input in "${scripts_inputs[@]}"; do
        ...
        time_log_file="./outputs/agg/time.log"
        { time ../agg_run.sh "$script_file" "$input_file" $ID "$log_file" "$agg_exec_file" "$time_log_file" >"$output_file"; } 2>"$time_file" #run file with input and direct to output
        echo $time_log_file | awk '{$1=$1};1'
        cat "${time_file}" >>$all_res_file
        echo "$script_file $(cat "$time_file")" | tee -a $mode_res_file
        ((ID++))
    done
