...
/- This is the concat_agg defined in Aggregators/Concat.lean
...
theorem concat_ordering_lt (f : ByteArray → ByteArray → ByteArray)
  ...
  ∀ xs ys a b c d,
    xs = a ++ b →
    ys = c ++ d →
    (concat_agg (f a c) (f b d)).size <= (xs ++ ys).size :=
  by
    intro xs ys a b c d hsplit hsplit1
    rw [concat_agg, hsplit, hsplit1]
    have h₁ := h a c
    have h₂ := h b d
...
/-- The output of concat_agg is smaller than its input -/
theorem concat_size (f: ByteArray → ByteArray)
  (h: ∀ input, (f input).size <= input.size) :
    ∀ str a b, str = a ++ b → (concat_agg (f a) (f b)).size <= str.size :=
  by
    intro s a b hsplit
    rw [concat_agg]
    rw [hsplit]
...
/-- This concat_agg uses List String instead of ByteArray.
...
theorem concat_none (f: ByteArray → ByteArray) (h : f ByteArray.empty = ByteArray.empty)
  : concat_agg (f ByteArray.empty) (f ByteArray.empty) = ByteArray.empty :=
  by
    rw [concat_agg]
    rw [h]
    rfl

/-- concat_agg preserves membership -/
...
theorem concat_correctness (f : ByteArray → ByteArray)
  ...
  ∀ input xs ys a b c d,
    ...
    xs = a ++ b →
    ys = c ++ d →
    concat_agg (f xs) (f ys) = concat_agg (f a) (f b) ++ concat_agg (f c) (f d) :=
  by
