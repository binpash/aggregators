...
for FILE in "${split_filelist[@]}"; do
    ...
    eval "cat "${FILE}" | "${CMD}" > ${CMD_SPLIT_FILE_DIR}/"${WITHOUTTXT}/${CMD_FILE_NAME}-${S_WITHOUTTXT}.txt"" # apply command to all split file
    filelist+="${CMD_SPLIT_FILE_DIR}/${WITHOUTTXT}/${CMD_FILE_NAME}-${S_WITHOUTTXT}.txt "
done
