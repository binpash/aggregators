...
printf "$LOG_PREFIX" ": Running aggregators for script: $SCRIPT and input file: $INPUT_FILE\n" >>"$DEBUG_LOG"
...
parse_simple() {
    echo "TESTING LOG: " "Parsing command pipeline from $SCRIPT" >>"$DEBUG_LOG"
...
find_agg() {
    ...
    ## Instruction for lean executables:
    ## To put in aggregators, name your aggregator executable command_flag
    ## and put it in the agg-synthesis directory.
    ## e.g. sort -n => sort-n; sort => sort_
    AGG_FILE_NO_FLAG="$CMD"
    if [ "${FLAG:0:1}" = "-" ]; then
        AGG_FILE="$CMD""$FLAG" # CHANGE AGG
    else
        AGG_FILE="$CMD"
    fi
    ...
    AGG_DIR="Aggregators"
    if [ -f "./Aggregators/${AGG_FILE}" ]; then
        echo "Aggregator Found ${AGG_FILE}" >&2
        echo "${AGG_FILE}"
    else
        echo "Aggregator Not Found ${AGG_FILE}" >&2
        echo ""
    fi
}
...
run() {
    ...
    echo "TESTING LOG: " "Parsing command pipeline finished; we have ${#CMDLIST[@]} command instances" >>"$DEBUG_LOG"

    local CURR_CMD_COUNT=0
    for CMD in "${CMDLIST[@]}"; do
        LOG_PREFIX="TESTING LOG: $CMD | "
        ...
        if [[ -z "$AGG" || "$AGG" == "NA" ]]; then
            ...
            echo "$LOG_PREFIX" "Running sequentially: aggregator not implemented " >>"$DEBUG_LOG"
